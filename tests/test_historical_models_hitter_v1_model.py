"""Milestone 32.3 -- preprocessing / candidate model tests for
historical_models.hitter_v1.model. Covers train-only imputation,
model-native missing-data handling, and reproducibility, parameterized
by feature-availability class."""
...
from historical_models.hitter_v1.model import CANDIDATES, build_imputed_preprocessor, build_pipeline, build_raw_preprocessor


def _synthetic_X_y(n=40, seed=0, with_nans=True):
    rng = np.random.default_rng(seed)
    data = {col: rng.uniform(0, 10, size=n) for col in AFTER_LINEUP_NUMERIC_FEATURE_COLUMNS}
    if with_nans:
        first_numeric = AFTER_LINEUP_NUMERIC_FEATURE_COLUMNS[0]
        data[first_numeric][: n // 4] = np.nan
...
def test_imputed_preprocessor_uses_train_statistics_only_not_validation():
    ...
    X_val, _ = _synthetic_X_y(n=10, seed=2)

    first_numeric = AFTER_LINEUP_NUMERIC_FEATURE_COLUMNS[0]
    train_median = X_train[first_numeric].median()

    preprocessor = build_imputed_preprocessor(AFTER_LINEUP_NUMERIC_FEATURE_COLUMNS, AFTER_LINEUP_CATEGORICAL_FEATURE_COLUMNS)
...
def test_raw_preprocessor_preserves_nan_for_tree_native_handling():
    ...
    preprocessor = build_raw_preprocessor(AFTER_LINEUP_NUMERIC_FEATURE_COLUMNS, AFTER_LINEUP_CATEGORICAL_FEATURE_COLUMNS)
    preprocessor.fit(X)
    transformed = preprocessor.transform(X)
    assert np.isnan(transformed[:, 0]).any()
...
def test_model_predictions_are_reproducible_under_fixed_seed():
    ...
    pipeline_a = build_pipeline(spec, params, AFTER_LINEUP_NUMERIC_FEATURE_COLUMNS, AFTER_LINEUP_CATEGORICAL_FEATURE_COLUMNS)
    pipeline_a.fit(X, y)
    preds_a = pipeline_a.predict(X)

    pipeline_b = build_pipeline(spec, params, AFTER_LINEUP_NUMERIC_FEATURE_COLUMNS, AFTER_LINEUP_CATEGORICAL_FEATURE_COLUMNS)
    pipeline_b.fit(X, y)
...
def test_one_hot_encoder_handles_unseen_category_without_raising():
    ...
    X_test[AFTER_LINEUP_CATEGORICAL_FEATURE_COLUMNS[0]] = "NEVER_SEEN_IN_TRAIN"
