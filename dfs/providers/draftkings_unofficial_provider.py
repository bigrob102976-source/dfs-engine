...
class DraftKingsUnofficialProvider(DFSSalaryProvider):
    ...
    def get_slate(
        ...
        for slate in day_slates:
            ...
            # provider-aware rules, never a structural one). NFL M1: each
            # sport gets its own Classic validator (same shared engine,
            # sport-specific roster/team/position constants) -- a sport
            # with no validator here keeps the original UNOFFICIAL_
            # DEVELOPMENT_SOURCE claim unchanged, out of scope until that
            # sport's own M1 is built.
