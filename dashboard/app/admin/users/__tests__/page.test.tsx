...
describe("AdminUsersPage", () => {
  it("renders every user with their subscription status", async () => {
    const user = createUser({ email: "listed@example.com", passwordHash: "h" });
  ...
  it("applies the search filter from the query string", async () => {
    createUser({ email: "findable@example.com", passwordHash: "h" });
    createUser({ email: "other@example.com", passwordHash: "h" });
