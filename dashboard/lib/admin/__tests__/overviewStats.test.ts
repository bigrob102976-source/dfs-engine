...
describe("computeAdminOverviewStats", () => {
  it("returns all-zero real figures (not fabricated placeholders) with no users at all", () => {
  ...
  it("computes MRR from actual active-subscriber counts times seeded plan prices", () => {
    const weeklyUser = createUser({ email: "weekly@example.com", passwordHash: "h" });
    insertSubscription({ userId: weeklyUser.id, planId: "weekly", status: "active" });
    const monthlyUser = createUser({ email: "monthly@example.com", passwordHash: "h" });
  ...
  it("does not count trialing or complimentary subscribers toward MRR", () => {
    const trialUser = createUser({ email: "trial@example.com", passwordHash: "h" });
    insertSubscription({ userId: trialUser.id, planId: "weekly", status: "trialing" });
    const compUser = createUser({ email: "comp@example.com", passwordHash: "h" });
    insertSubscription({ userId: compUser.id, planId: "monthly", status: "complimentary" });
  ...
  it("computes trial conversion rate from trial-ever vs currently-active", () => {
    ...
    const converted = createUser({ email: "converted@example.com", passwordHash: "h" });
    ...
    const stillTrialing = createUser({ email: "stilltrial@example.com", passwordHash: "h" });
    insertSubscription({
